...
from elasticsearch_dsl import DocType, Text, Date, Keyword, Search, tokenizer, analyzer
...
my_analyzer = analyzer('my_analyzer', filter=['lowercase', 'standard', 'snowball'],
                       tokenizer=tokenizer('trigram', 'nGram', min_gram=3, max_gram=3),)


class DocumentIndex(DocType):
    '''
    DocType model-like wrapper around model Document
    '''
...
def search(text):
    ...
    '''
    s = Search().query('multi_match', query=text, fields=['title', 'description', 'text'])
    response = s.execute()
    return response
